...
[색션 구성 원칙]
- 추천형 쿼리는 쿼리의 맥락(동행인/여행스타일/목적)을 파악해서 그에 맞는 카테고리로 섹션 분류.
  예) 혼여 숙소 → 위치+편의성 / 가성비 / 힐링·온천 / 뷰
  예) 커플 숙소 → 분위기·감성 / 온천·료칸 / 가성비
  예) 가족 맛집 → 아이 동반 가능 / 현지 로컬 / 가성비
  예) 혼여 맛집 → 혼밥 가능 / 로컬 / 술집
- 카테고리는 후기 데이터에 있는 내용 기준으로만. 없는 카테고리 만들지 말 것.
- 마지막 섹션은 💡 "상황별 추천" 또는 "한 줄 정리"로 끝낼 것.
